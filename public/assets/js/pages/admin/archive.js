jQuery(document).ready(function($){
	...
	var dataTable = $('#tbl1')
		...
		.DataTable({
				...
				
				{
					"mRender": function ( data, type, row ) {
						...
						if(row.attachment){
							...
							html+= '<a href="'+url+'" target="_blank" class="open-pdf">View attachment</a>&nbsp;&nbsp;';
						}
						html+= '<a href="/admin/archive/'+row.ID+'" class="">Update</a>&nbsp;&nbsp;';
			...
			"ajax":{
				"url":window.APP_API_URL+'/api/admin/archives',
				"type":"GET",
				"headers":getHeader(),	
	...
	function bindDeleteBtn(){
		$('#tbl1 .btn-dangerx').off('click').on('click', function(e){
			...
			$.ajax({
				type: "DELETE",
				headers:getHeader(),	
				url: window.APP_API_URL+'/api/admin/archives/'+selid,
				data: {
					id:selid
				},
