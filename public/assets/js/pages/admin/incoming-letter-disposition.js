jQuery(document).ready(function($){
		...
		$.ajax({
			...
			success:function (data) {
				...
				if(data.status==1){
					...
					$('#frmDisposition').attr('action',window.APP_API_URL+'/api/admin/incoming-letters/'+id+'/dispositions');
					$('input[name="return-url"]').val('/admin/incoming-letter/'+id+'/disposition');
	...
	function getAllFlows(){
		...
		var url = window.APP_API_URL+'/api/admin/roles/'+id+'/disposition-flow/search?pageSize=100';
		$.ajax({
			url, 
			headers:getHeader(),			
			success:function (data) {
	...
	function getAllDispositions(incomingLetterId){
		var id = localStorage.roleID;
		var url = window.APP_API_URL+'/api/admin/incoming-letters/'+incomingLetterId+'/dispositions';
		$.ajax({
			url, 
			headers:getHeader(),			
			success:function (data) {
