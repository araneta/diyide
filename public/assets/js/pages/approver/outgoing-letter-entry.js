		...
		$.ajax({
			...
			success:function (data) {
				...
				if(data.status==1){
					...
					$('#existingContent a').attr('href', window.APP_API_URL+'/api/approver/file/'+filecontent+'/'+fname);
					if(filecontent){
					...
					var fname = letter.no.replaceAll('\/','-')+'.docx';
					var urlContent = window.APP_API_URL+'/api/approver/file/'+letter.docxFile+'/'+fname;
					var lnkContent = '<a href="'+urlContent+'" target="_blank">Download</a>';
					$('#txtContent').html(lnkContent);
					
					$('#txtAttachment a').attr('href', window.APP_API_URL+'/api/approver/file/'+file+'/'+fname);
