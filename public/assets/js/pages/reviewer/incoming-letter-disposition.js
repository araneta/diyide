jQuery(document).ready(function($){
	...
	if(id){
		getAllFlows();
		var url = window.APP_API_URL+'/api/reviewer/incoming-letters/'+id;
		$.ajax({
			url, 
			headers:getHeader(),			
			success:function (data) {
				...
				if(data.status==1){
					...
					$('#frmDisposition').attr('action',window.APP_API_URL+'/api/reviewer/incoming-letters/'+id+'/dispositions');
					$('input[name="return-url"]').val('/reviewer/incoming-letter/'+id+'/disposition');
	...
	function getAllFlows(){
		...
		var url = window.APP_API_URL+'/api/reviewer/disposition-flow/search?pageSize=100';
		$.ajax({
			url, 
			headers:getHeader(),			
			success:function (data) {
	...
	function getAllDispositions(incomingLetterId){
		var id = localStorage.roleID;
		var url = window.APP_API_URL+'/api/reviewer/incoming-letters/'+incomingLetterId+'/dispositions';
		$.ajax({
			url, 
			headers:getHeader(),			
			success:function (data) {
