jQuery(document).ready(function($){
		...
		$.ajax({
			...
			success:function (data) {
				...
				if(data.status==1){
					...
					$('#existingContent a').attr('href', window.APP_API_URL+'/api/reviewer/file/'+filecontent+'/'+fname);
					if(filecontent){
					...
					$('#existingAttachment a').attr('href', window.APP_API_URL+'/api/reviewer/file/'+file+'/'+fname);
					if(file){
					...
					//view 
					$('#txtNo').html(letter.no);
					$('#txtLetterDate').html(toLocalDate(letter.letterDate));
					$('#txtSubject').html(letter.subject);
					$('#txtDestination').html(letter.destination);
					
					var fname = letter.no.replaceAll('\/','-')+'.docx';
					var urlContent = window.APP_API_URL+'/api/reviewer/file/'+letter.docxFile+'/'+fname;
					var lnkContent = '<a href="'+urlContent+'" target="_blank">Download</a>';
					$('#txtContent').html(lnkContent);
					
					$('#txtAttachment a').attr('href', window.APP_API_URL+'/api/reviewer/file/'+file+'/'+fname);
					$('#versionNo').val(letter.versionNo);
					$('#btnApprove').click(function(e){
						var url = $(e.target).attr('formaction');
						$('#frmReview').attr('action',url);
						$('#frmReview').submit();
					});
	...
	function getAllVersions(outgoingLetterId){
		...
		var url = window.APP_API_URL+'/api/reviewer/outgoing-letters/'+outgoingLetterId+'/versions';
		$.ajax({
			url, 
			headers:getHeader(),			
			success:function (data) {
				...
				if(data.status==1){
					...
					$('.lnkversion').on('click',function(e){
						...
						for(var i=0;i<versions.length;i++){
							var vers = versions[i];
							if(vers.ID==id){
								var url = window.APP_API_URL+'/api/reviewer/file/'+vers.attachment+'/'+vers.fileName;
								var lnk = '<a href="'+url+'" target="_blank" class="open-pdf">View</a>';
								
								var fname = vers.no.replaceAll('\/','-')+'.docx';
								var urlContent = window.APP_API_URL+'/api/reviewer/file/'+vers.docxFile+'/'+fname;
								var lnkContent = '<a href="'+urlContent+'" target="_blank">Download</a>';
								
								$('#detailDlg #seltxtNo').html(vers.no);
								$('#detailDlg #seltxtLetterDate').html(formatTime(vers.letterDate));
								$('#detailDlg #seltxtSubject').html(vers.subject);
								$('#detailDlg #seltxtDestination').html(vers.no);
								$('#detailDlg #seltxtContent').html(lnkContent);
								$('#detailDlg #seltxtAttachment').html(lnk);
