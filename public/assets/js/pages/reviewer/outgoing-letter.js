jQuery(document).ready(function($){
	...
	var dataTable = $('#tbl')
		...
		.DataTable({
			...
			"aoColumnDefs": [
				...
				
				{
					"mRender": function ( data, type, row ) {
						...
						html+= '<li><a href="/reviewer/outgoing-letter/'+row.ID+'/view" class="dropdown-item">View</a></li>';
						if(row.approvalStatus != 3){
							html+= '<li><a href="/reviewer/outgoing-letter/'+row.ID+'" class="dropdown-item">Update</a></li>';
						}
						//html+= '<li><a href="/reviewer/outgoing-letter/'+row.ID+'/print" class="dropdown-item"  target="_blank">Cetak</a></li>';
						var fname = row.no.replaceAll('\/','-')+'.docx';
						var url = window.APP_API_URL+'/api/reviewer/file/'+row.docxFile+'/'+fname;
						html += '<li><a href="'+url+'"  target="_blank" class="dropdown-item" >Cetak</a></li>';
						if(row.approvalStatus == 3){//approved
							
							if(!row.isArchived){
								
								html += '<li><a href="/reviewer/outgoing-letter/'+row.ID+'/archive" class="dropdown-item">Arsipkan</a></li>';
							}else{
								if(row.archiveFileName){//has archived
									var url = window.APP_API_URL+'/api/reviewer/file/'+row.archiveAttachment+'/'+row.archiveFileName;
			...
			"ajax":{
				"url":window.APP_API_URL+'/api/reviewer/outgoing-letters',
				"type":"GET",
				"headers":getHeader(),	
